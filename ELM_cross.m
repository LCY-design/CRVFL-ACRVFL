...
for i = 1 : 10
    ...
    train = ~test;
    %1份测试，9份训练
    test_set = dataset(test, 1 : data_c-1);
    test_label =dataset(test,data_c);
    
    train_set = dataset(train, 1 : data_c-1);
...
fileID = fopen('resultELM_Cross.txt','a');
...
fileID = fopen('resultELM_Crossl.txt','a');
  fprintf(fileID,'Result for  hiddenn=%d,ActivationFunction=%s:\n', hiddenn,ActivationFunction);
   fprintf(fileID,'%12.4f',AverageAcctesting);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageAcctraining);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageTrainingTime);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultELM_Crosste.txt','a');%以 a 的方式创建名为 result.txt 的文件（在文件末端添加数据）？
       fprintf(fileID,'%12.4f',AverageAcctesting);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultELM_Crosstre.txt','a');
       fprintf(fileID,'%12.4f',AverageAcctraining);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultELM_Crosstrt.txt','a');
      fprintf(fileID,'%12.4f',AverageTrainingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultELM_Crosstt.txt','a');
      fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
end
%close(wb);
