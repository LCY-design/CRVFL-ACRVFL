...
AverageAcctesting
fileID = fopen('resultRVFL_Cross.txt','a');
...
fileID = fopen('resultRVFL_Crossl.txt','a');
  fprintf(fileID,'Result for  hiddenn=%d,ActivationFunction=%s:\n', hiddenn,ActivationFunction);
   fprintf(fileID,'%12.4f',AverageAcctesting);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageAcctraining);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageTrainingTime);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultRVFL_Crosste.txt','a');%以 a 的方式创建名为 result.txt 的文件（在文件末端添加数据）？
       fprintf(fileID,'%12.4f',AverageAcctesting);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultRVFL_Crosstre.txt','a');
       fprintf(fileID,'%12.4f',AverageAcctraining);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultRVFL_Crosstrt.txt','a');
      fprintf(fileID,'%12.4f',AverageTrainingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultRVFL_Crosstt.txt','a');
      fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
end
%close(wb);
