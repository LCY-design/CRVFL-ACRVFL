...
fileID = fopen('resultELM_Real.txt','a');
...
fileID = fopen('resultELM_Realchannel.txt','a');
  fprintf(fileID,'Result for  hiddenn=%d,ActivationFunction=%s:\n', hiddenn,ActivationFunction);
   fprintf(fileID,'%12.4f',Averagermstesting);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',Averagermstraining);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageTrainingTime);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultELM_Realchannelte.txt','a');%以 a 的方式创建名为 result.txt 的文件（在文件末端添加数据）？
       fprintf(fileID,'%12.4f',Averagermstesting);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultELM_Realchanneltre.txt','a');
       fprintf(fileID,'%12.4f',Averagermstraining);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultELM_Realchanneltrt.txt','a');
      fprintf(fileID,'%12.4f',AverageTrainingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultELM_Realchanneltt.txt','a');
      fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
end
close(wb);
...
z1 =csvread('resultELM_Realchannelte.txt');
plot(x,z1,'r');
grid on
legend('ELM_Real')
...
subplot(2,2,2)
x =10:10:200;
z2=csvread('resultELM_Realchanneltrt.txt');
plot(x,z2,'b');
grid on
legend('ELM_Real')
xlabel('the number of hidden nodes')
