...
tic;
weight = 0.1*(2*rand(hiddenn,dim)-ones(hiddenn,dim))+j*0.1*(2*rand(hiddenn,dim)-ones(hiddenn,dim));
bias = 0.1*rand(1,hiddenn)+j*0.1*rand(1,hiddenn);
ind=ones(1,N);
BiasMatrix=bias(ind,:);
...
fileID = fopen('resultCRVFL-RA.txt','a');
...
fileID = fopen('resultCRVFL-RA.txt','a');
  fprintf(fileID,'Result for  hiddenn=%d,ActivationFunction=%s:\n', hiddenn,ActivationFunction);
   ...
   fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultCRVFL-RAte.txt','a');%以 a 的方式创建名为 result.txt 的文件（在文件末端添加数据）？
       fprintf(fileID,'%12.4f',Averagermstesting);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultCRVFL-RAtre.txt','a');
       fprintf(fileID,'%12.4f',Averagermstraining);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultCRVFL-RAtrt.txt','a');
      fprintf(fileID,'%12.4f',AverageTrainingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultCRVFL-RAtt.txt','a');
      fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
end
close(wb);
...
subplot(2,2,1)
x =1:1:1000;%hidden nodes
z1 =csvread('resultCRVFL-RAte.txt');
plot(x,z1,'r');
grid on
legend('CRVFL-RA')
...
subplot(2,2,2)
x =1:1:1000;
z2=csvread('resultCRVFL-RAtrt.txt');
plot(x,z2,'b');
grid on
legend('CRVFL-RA')
xlabel('the number of hidden nodes')
