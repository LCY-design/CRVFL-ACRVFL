...
H2=[real(train_set),imag(train_set)];
H=[H2,H1];
%H=[real(H2),imag(H2)];
%H=[real(H2),imag(H2)];
...
H_test2=[real(test_set),imag(test_set)];
...
fileID = fopen('resultInput_RA.txt','a');
...
fileID = fopen('resultInput_RA.txt','a');
  fprintf(fileID,'Result for  hiddenn=%d,ActivationFunction=%s:\n', hiddenn,ActivationFunction);
   ...
   fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultInput_RAte.txt','a');%以 a 的方式创建名为 result.txt 的文件（在文件末端添加数据）？
       fprintf(fileID,'%12.4f',Averagermstesting);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultInput_RAtre.txt','a');
       fprintf(fileID,'%12.4f',Averagermstraining);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultInput_RAtrt.txt','a');
      fprintf(fileID,'%12.4f',AverageTrainingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultInput_RAtt.txt','a');
      fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
end
close(wb);
...
z1 =csvread('resultInput_RAte.txt');
plot(x,z1,'r');
grid on
legend('Input_RA')
...
subplot(2,2,2)
x =10:10:200;
z2=csvread('resultInput_RAtrt.txt');
plot(x,z2,'b');
grid on
legend('Input_RA')
xlabel('the number of hidden nodes')
