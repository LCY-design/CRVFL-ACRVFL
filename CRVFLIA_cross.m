...
dataset=xlsread('Page Blocks Classification.xlsx');
%num=table2array(num);
% X=num(:,1:12);
% Y=num(:,13);
...
H_test2=[real(P_test),imag(P_test)];
H_test=[H_test2,H_test1];
...
fileID = fopen('resultCRVFLIA_Cross.txt','a');
...
fileID = fopen('resultCRVFLIA_Crossl.txt','a');
  fprintf(fileID,'Result for  hiddenn=%d,ActivationFunction=%s:\n', hiddenn,ActivationFunction);
   fprintf(fileID,'%12.4f',AverageAcctesting);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageAcctraining);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageTrainingTime);%fprintf(fileID,'\n');
   fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultCRVFLIA_Crosste.txt','a');%以 a 的方式创建名为 result.txt 的文件（在文件末端添加数据）？
       fprintf(fileID,'%12.4f',AverageAcctesting);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultCRVFLIA_Crosstre.txt','a');
       fprintf(fileID,'%12.4f',AverageAcctraining);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultCRVFLIA_Crosstrt.txt','a');
      fprintf(fileID,'%12.4f',AverageTrainingTime);fprintf(fileID,'\n');
fclose(fileID);
fileID = fopen('resultCRVFLIA_Crosstt.txt','a');
      fprintf(fileID,'%12.4f',AverageTestingTime);fprintf(fileID,'\n');
fclose(fileID);
end
%close(wb);
